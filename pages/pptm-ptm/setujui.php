...
// status_workflow_id = 2 bermaksud "Menunggu PPTM" — itu sahaja yang
// sah untuk diluluskan di sini. status_aset_id (Aktif/Rosak/dll) ialah
// status fizikal aset, tidak berkaitan dengan peringkat kelulusan.
// Turut skop kepada wilayah PPTM/PTM supaya tidak boleh luluskan aset
// wilayah lain.
...
if (!$aset) {
    $_SESSION['flash_error'] = "Aset tidak boleh diluluskan pada masa ini (mungkin sudah diproses oleh pengguna lain).";
    header("Location: pengesahan.php");
    exit;
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        // ── FIX: Update status_workflow_id (4 = Menunggu KW), BUKAN status_aset_id ──
        // status_aset_id (kondisi fizikal aset) tidak disentuh langsung.
        $update_stmt = mysqli_prepare($conn, "UPDATE aset SET status_workflow_id = 4 WHERE aset_id = ?");
        ...
        // ── Log audit trail ke log_workflow ──
        $log_stmt = mysqli_prepare(
            ...
            "INSERT INTO log_workflow
             ...
             VALUES
                (?, 'Lulus PPTM', ?, 4, 'Diluluskan peringkat PPTM/PTM', ?)"
        );
        mysqli_stmt_bind_param($log_stmt, "iii", $aset_id, $old_status, $pengguna_id);
        mysqli_stmt_execute($log_stmt);

        // ── Notifikasi kepada Ketua Wilayah dalam wilayah yang sama ──
        // (Nota: laraskan $url di bawah ikut laluan sebenar halaman Ketua Wilayah anda)
        $kw_stmt = mysqli_prepare(
            $conn,
            "SELECT p.pengguna_id
             FROM pengguna p
             JOIN peranan r ON p.peranan_id = r.peranan_id
             WHERE r.nama_peranan = 'Ketua Wilayah'
               AND p.wilayah_id = ?
               AND p.status_pengguna_id = 1"
        );
        mysqli_stmt_bind_param($kw_stmt, "i", $wilayah_id);
        mysqli_stmt_execute($kw_stmt);
        $kw_list = mysqli_fetch_all(mysqli_stmt_get_result($kw_stmt), MYSQLI_ASSOC);
        mysqli_stmt_close($kw_stmt);

        $no_pendaftaran = $aset['no_pendaftaran'];
        $mesej = "Aset {$no_pendaftaran} telah diluluskan PPTM/PTM, menunggu kelulusan Ketua Wilayah.";
        $url = "/jdtis_asset/pages/ketua-wilayah/lihat.php?id=" . intval($aset_id);
        ...
        foreach ($kw_list as $kw) {
            $penerima_id = intval($kw['pengguna_id']);
        ...
        logActivity($conn, 'Pengesahan Aset', "Aset " . $no_pendaftaran . " diluluskan oleh " . $_SESSION['peranan']);

        header("Location: pengesahan.php?success=approved");
        exit;
    } catch (Exception $e) {
        mysqli_rollback($conn);
        die("Ralat semasa meluluskan aset. Sila cuba lagi.");
    }
}
    ...
    <div class="container-fluid">
        ...
        <aside class="col-md-3 col-xl-2 sidebar p-4">
            ...
            <nav class="nav flex-column">
                ...
                </a><h1 class="page-title">Luluskan Aset</h1><p class="page-subtitle">Sahkan aset sebelum diteruskan kepada Ketua Wilayah.</p></div><div class="page-chip"><i class="bi bi-check-circle"></i> Kelulusan PPTM</div></div>

                <div class="card decision-card">
                    <div class="card-body p-4">
                        <h3 class="fw-bold mb-4"><i class="bi bi-check-circle-fill text-success me-2"></i>Pengesahan Kelulusan</h3>
                        
                        <div class="alert alert-info mb-4">
                            <i class="bi bi-info-circle"></i> Anda akan meluluskan aset berikut. Aset ini akan diteruskan ke peringkat <strong>Kelulusan Ketua Wilayah</strong>.
                        </div>
                        ...
                        <form method="POST">
                            ...
                            <div class="alert alert-warning">
                                <i class="bi bi-exclamation-triangle"></i> <strong>Perhatian:</strong> Tindakan ini tidak boleh dibatalkan. Sila pastikan maklumat aset adalah sah dan lengkap sebelum meluluskan.
                            </div>

                            <div class="d-flex flex-wrap gap-2 mt-4">
                                <button type="submit" class="btn btn-success btn-lg" onclick="return confirm('Adakah anda pasti ingin meluluskan aset ini?');">
