...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($nama_wilayah === '') {
            ...
            $check_query = "SELECT COUNT(*) AS count FROM wilayah WHERE nama_wilayah = ? AND wilayah_id != ?";
            $check_stmt = mysqli_prepare($conn, $check_query);
            mysqli_stmt_bind_param($check_stmt, 'si', $nama_wilayah, $wilayah_id);
            ...
            if ((int) ($check_row['count'] ?? 0) > 0) {
                ...
                $update_query = "UPDATE wilayah SET nama_wilayah = ?, jenis = ?, kod_wilayah = ? WHERE wilayah_id = ?";
                $update_stmt = mysqli_prepare($conn, $update_query);
                mysqli_stmt_bind_param($update_stmt, 'sssi', $nama_wilayah, $jenis, $kod_wilayah, $wilayah_id);

                if (mysqli_stmt_execute($update_stmt)) {
                    logActivity($conn, 'Kemaskini Wilayah', "Wilayah dikemaskini: $nama_wilayah");
                    header('Location: index.php?success=' . urlencode('Wilayah berjaya dikemaskini'));
                    exit;
                }
...
<head>
    ...
    <title>Edit Wilayah - Super Admin JTDIS</title>
                ...
                <div class="card-body p-4">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama_wilayah" class="form-control" value="<?php echo escapeOutput($wilayah['nama_wilayah']); ?>" required>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Jenis *</label>
                            <select name="jenis" class="form-select" required>
                                <option value="ibu_pejabat" <?php echo $wilayah['jenis'] === 'ibu_pejabat' ? 'selected' : ''; ?>>Ibu Pejabat</option>
                                <option value="wilayah" <?php echo $wilayah['jenis'] === 'wilayah' ? 'selected' : ''; ?>>Wilayah</option>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Kod Wilayah</label>
                            <input type="text" name="kod_wilayah" class="form-control" value="<?php echo escapeOutput($wilayah['kod_wilayah'] ?? ''); ?>">
                        </div>
