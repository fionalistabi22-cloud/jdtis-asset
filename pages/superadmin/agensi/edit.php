...
$query = "SELECT * FROM agensi WHERE agensi_id = ?";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($nama_agensi === '') {
            ...
            $check_query = "SELECT COUNT(*) AS count FROM agensi WHERE nama_agensi = ? AND agensi_id != ?";
            $check_stmt = mysqli_prepare($conn, $check_query);
            mysqli_stmt_bind_param($check_stmt, 'si', $nama_agensi, $agensi_id);
            ...
            if ((int) ($check_row['count'] ?? 0) > 0) {
                ...
                $update_query = "UPDATE agensi SET nama_agensi = ?, daerah_id = ?, wilayah_id = ?, jenis_agensi = ?, catatan = ? WHERE agensi_id = ?";
                $update_stmt = mysqli_prepare($conn, $update_query);
                mysqli_stmt_bind_param($update_stmt, 'siissi', $nama_agensi, $daerah_id, $wilayah_id, $jenis_agensi, $catatan, $agensi_id);

                if (mysqli_stmt_execute($update_stmt)) {
                    logActivity($conn, 'Kemaskini Agensi', "Agensi dikemaskini: $nama_agensi");
                    header('Location: index.php?success=' . urlencode('Agensi berjaya dikemaskini'));
                    exit;
                }
...
<head>
    ...
    <title>Edit Agensi - Super Admin JTDIS</title>
                ...
                <div class="card-body p-4">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="nama_agensi" class="form-control" value="<?php echo escapeOutput($agensi['nama_agensi']); ?>" required>
                        </div>
                        <div class="row">
                            <div class="col-md-6 mb-3">
                                <label class="form-label">Jenis *</label>
                                <select name="jenis_agensi" class="form-select" required>
                                    <option value="kementerian" <?php echo $agensi['jenis_agensi'] === 'kementerian' ? 'selected' : ''; ?>>Kementerian</option>
                                    <option value="jabatan" <?php echo $agensi['jenis_agensi'] === 'jabatan' ? 'selected' : ''; ?>>Jabatan</option>
                                    <option value="pejabat_daerah" <?php echo $agensi['jenis_agensi'] === 'pejabat_daerah' ? 'selected' : ''; ?>>Pejabat Daerah</option>
                                    <option value="lain" <?php echo $agensi['jenis_agensi'] === 'lain' ? 'selected' : ''; ?>>Lain</option>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="wilayah_id" class="form-select">
                                    ...
                                    <?php foreach ($wilayah_list as $wilayah): ?>
                                        <option value="<?php echo (int) $wilayah['wilayah_id']; ?>" <?php echo ((int) ($agensi['wilayah_id'] ?? 0) === (int) $wilayah['wilayah_id']) ? 'selected' : ''; ?>><?php echo escapeOutput($wilayah['nama_wilayah']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        <div class="mb-3">
                            ...
                            <select name="daerah_id" class="form-select">
                                ...
                                <?php foreach ($daerah_list as $daerah): ?>
                                    <option value="<?php echo (int) $daerah['daerah_id']; ?>" <?php echo ((int) ($agensi['daerah_id'] ?? 0) === (int) $daerah['daerah_id']) ? 'selected' : ''; ?>><?php echo escapeOutput($daerah['nama_daerah']); ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="mb-3">
                            <label class="form-label">Catatan</label>
                            <textarea name="catatan" class="form-control" rows="4"><?php echo escapeOutput($agensi['catatan'] ?? ''); ?></textarea>
                        </div>
