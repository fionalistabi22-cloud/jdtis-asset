    ...
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($nama_penuh === '') {
            ...
            if ((int) ($check_row['count'] ?? 0) > 0) {
                ...
                mysqli_stmt_bind_param($insert_stmt, 'sssiiiii', $nama_penuh, $emel, $password_hash, $peranan_id, $wilayah_id, $daerah_id, $agensi_id, $status_pengguna_id);

                if (mysqli_stmt_execute($insert_stmt)) {
                    logActivity($conn, 'Tambah Pengguna', "Pengguna baru ditambah: $emel ($nama_penuh)");
                    header('Location: index.php?success=' . urlencode('Pengguna berjaya ditambah'));
                    exit;
                }
...
<head>
    ...
    <title>Tambah Pengguna - Super Admin JTDIS</title>
            ...
            <div class="card">
                <div class="card-body p-4">
                    <form method="POST">
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="peranan_id" class="form-select" required>
                                    ...
                                    <?php foreach ($roles as $role): ?>
                                        <option value="<?php echo (int) $role['peranan_id']; ?>"><?php echo escapeOutput($role['nama_peranan']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="wilayah_id" class="form-select">
                                    ...
                                    <?php foreach ($wilayah_list as $w): ?>
                                        <option value="<?php echo (int) $w['wilayah_id']; ?>"><?php echo escapeOutput($w['nama_wilayah']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="daerah_id" class="form-select">
                                    ...
                                    <?php foreach ($daerah_list as $d): ?>
                                        <option value="<?php echo (int) $d['daerah_id']; ?>"><?php echo escapeOutput($d['nama_daerah']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                            <div class="col-md-4 mb-3">
                                ...
                                <select name="agensi_id" class="form-select">
                                    ...
                                    <?php foreach ($agensi_list as $a): ?>
                                        <option value="<?php echo (int) $a['agensi_id']; ?>"><?php echo escapeOutput($a['nama_agensi']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                            ...
                            <div class="col-md-6 mb-3">
                                ...
                                <select name="status_pengguna_id" class="form-select">
                                    <?php foreach ($status_list as $status): ?>
                                        <option value="<?php echo (int) $status['status_pengguna_id']; ?>" <?php echo ((int) $status['status_pengguna_id'] === 1) ? 'selected' : ''; ?>><?php echo escapeOutput($status['status']); ?></option>
                                    <?php endforeach; ?>
                                </select>
                            </div>
                        </div>
                        ...
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Simpan Pengguna</button>
