...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (!verifyCSRFToken($_POST['csrf_token'] ?? '')) {
        ...
        if ($nama_daerah === '') {
            ...
            $error = 'Sila pilih wilayah';
        } else {
            $check_query = "SELECT COUNT(*) AS count FROM daerah WHERE nama_daerah = ?";
            $check_stmt = mysqli_prepare($conn, $check_query);
            mysqli_stmt_bind_param($check_stmt, 's', $nama_daerah);
            ...
            if ((int) ($check_row['count'] ?? 0) > 0) {
                ...
                mysqli_stmt_bind_param($insert_stmt, 'sis', $nama_daerah, $wilayah_id, $kod_daerah);

                if (mysqli_stmt_execute($insert_stmt)) {
                    logActivity($conn, 'Tambah Daerah', "Daerah baru ditambah: $nama_daerah");
                    header('Location: index.php?success=' . urlencode('Daerah berjaya ditambah'));
                    exit;
                }
...
<head>
    ...
    <title>Tambah Daerah - Super Admin JTDIS</title>
                ...
                <div class="card-body p-4">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="kod_daerah" class="form-control" placeholder="Contoh: KBL">
                        </div>
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Simpan Daerah</button>
            ...
            <div class="card" style="max-width: 700px;">
                <div class="card-body p-4">
                    <form method="POST">
                        ...
                        <div class="mb-3">
                            <label class="form-label">Kod Daerah</label>
                            <input type="text" name="kod_daerah" class="form-control" placeholder="Contoh: KBL, KUD">
                        </div>
                        <button type="submit" class="btn btn-primary"><i class="bi bi-check-lg"></i> Simpan Daerah</button>
                        <a href="index.php" class="btn btn-secondary">Batal</a>
                    </form>
