...
// Peranan: Agen IT ATAU Juruteknik (kedua-dua boleh guna endpoint re-submit)
if (!in_array($_SESSION['peranan'] ?? '', ['Agen IT', 'Juruteknik'], true)) {
...
$semak_query = "SELECT aset_id, no_pendaftaran, status_workflow_id, wilayah_id, pengguna_id_daftar
               FROM aset
               WHERE aset_id = ?
                 AND pengguna_id_daftar = ?
                 AND status_workflow_id IN (1, 3)";
...
try {
    ...
    // 3) INSERT notifikasi kepada semua PPTM/PTM users dalam wilayah yang sama
    //    AND status_pengguna_id = 1
