...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Verify asset ownership (must be registered by this user in same agensi)
    $check_query = "SELECT a.no_pendaftaran FROM aset a WHERE a.aset_id = ? AND a.pengguna_id_daftar = ? AND a.agensi_id = ?";
    $check_stmt = mysqli_prepare($conn, $check_query);
    mysqli_stmt_bind_param($check_stmt, "iii", $aset_id, $pengguna_id, $agensi_id);
    ...
    // Delete asset
    $delete_query = "DELETE FROM aset WHERE aset_id = ? AND pengguna_id_daftar = ? AND agensi_id = ?";
    $delete_stmt = mysqli_prepare($conn, $delete_query);
    mysqli_stmt_bind_param($delete_stmt, "iii", $aset_id, $pengguna_id, $agensi_id);
    
    if (mysqli_stmt_execute($delete_stmt)) {
        logActivity($conn, 'Hapus Aset', "Aset {$aset['no_pendaftaran']} dihapuskan oleh Agen IT");
    ...
    $query = "SELECT a.* FROM aset a WHERE a.aset_id = ? AND a.pengguna_id_daftar = ? AND a.agensi_id = ?";
    $stmt = mysqli_prepare($conn, $query);
    mysqli_stmt_bind_param($stmt, "iii", $aset_id, $pengguna_id, $agensi_id);
            ...
            <div class="col-md-9 col-xl-10 main-content">
                <div class="page-hero">
                    <div>
                        ...
                        <h1 class="page-title">Hapus Aset</h1>
                        <p class="page-subtitle">Semak aset dengan teliti sebelum pemadaman diteruskan.</p>
                    </div>
                    <div class="page-chip"><i class="bi bi-trash3"></i> Agen IT</div>
                </div>
                    ...
                    <div class="card-body">
                        ...
                        <table class="table table-borderless">
                            ...
                            <tr>
                                ...
                                <td><?php echo escapeOutput($aset['processor'] ?? '-'); ?></td>
                            </tr>
                            <tr>
                                <td><strong>RAM:</strong></td>
                                <td><?php echo escapeOutput($aset['ram'] ?? '-'); ?></td>
                            </tr>
                            <tr>
                                <td><strong>Storan:</strong></td>
                                <td><?php echo escapeOutput($aset['cakera_keras'] ?? '-'); ?></td>
                            </tr>
                        </table>
