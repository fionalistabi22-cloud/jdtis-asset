...
// Protect page - role Agen IT sahaja
...
// Get assets registered by this Agen IT (workflow)
// Semua column di bawah WUJUD dalam table `aset` (jtdis_asset.sql):
//   status_aset_id, status_workflow_id, spesifikasi_pencetak, cakera_keras, dll.
// FIX: tambah a.status_aset_id (punca "Undefined array key") + join status_aset
$aset_query = "SELECT 
                a.aset_id,
                a.no_pendaftaran,
                a.jenis_aset,
...
// FIX: Statistik ikut nilai status_aset dalam DB
// status_aset: 1=Aktif, 2=Rosak, 3=Selenggara, 4=Hilang, 5=Dilupuskan
// status_workflow: 2=Menunggu PPTM
$jumlah_aset = count($aset_list);
$pending = count(array_filter($aset_list, function($a) {
    return intval($a['status_workflow_id'] ?? 0) === 2;
}));
$active = count(array_filter($aset_list, function($a) {
    return intval($a['status_aset_id'] ?? 0) === 1;
}));
$inactive = count(array_filter($aset_list, function($a) {
    return intval($a['status_aset_id'] ?? 0) !== 1;
}));
    ...
    <div class="container-fluid">
        <div class="row">
            ...
            <div class="col-md-9 col-xl-10 main-content">
                <div class="page-hero">
                    <div>
                        <h1 class="page-title">Senarai Aset Agen IT</h1>
                    ...
                    <a href="tambah.php" class="btn btn-primary btn-lg">
                        <i class="bi bi-plus-circle me-1"></i> Daftar Aset Baharu
                    </a>
                </div>
                ...
                <div class="row g-3 mb-4">
                    <div class="col-sm-6 col-xl-3">
                        <div class="stat-card stat-blue">
                            ...
                            <div class="stat-value"><?php echo $jumlah_aset; ?></div>
                ...
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-0 fw-bold">Rekod Aset Terdaftar</h5>
                    </div>
                                        ...
                                        <?php foreach ($aset_list as $aset): ?>
                                            <tr>
                                                ...
                                                <td>
                                                    ...
                                                    // spesifikasi_pencetak (wujud dlm DB) — papar untuk jenis Pencetak
                                                    if (!empty($aset['spesifikasi_pencetak'])) $specs .= '<strong>Pencetak:</strong> ' . escapeOutput($aset['spesifikasi_pencetak']) . ' ';
                                                ...
                                                <!-- Status Aset (dari table status_aset) -->
                                                <td>
                                                    <?php
                                                    $warna_aset = $aset['warna_status_aset'] ?? 'secondary';
                                                    $label_aset = $aset['nama_status_aset'] ?? '-';
                                                    echo '<span class="badge badge-status bg-' . escapeOutput($warna_aset) . '">' . escapeOutput($label_aset) . '</span>';
                                                    ?>
                                                </td>

                                                <!-- Status Workflow (dari table status_workflow) -->
                                                <td>
                                                   <?php
                                                ...
                                                <td>
                                                    <div class="btn-group" role="group">
                                                        ...
                                                        <?php $status_workflow_id = intval($aset['status_workflow_id'] ?? 0); ?>
